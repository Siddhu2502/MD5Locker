...
def leftRotate(x, amount):
	x &= 0xFFFFFFFF # 0xFFFFFFFF means -1
	return (x << amount | x >> (32-amount)) & 0xFFFFFFFF


# STEP 4: process the message in 16-word blocks
# Message block stored in buffers is processed in the follg general manner:
# A = B + rotate left by some amount<-(A + func(B, C, D) + additive constant + 1 of the 
# 16 32-bit(4 byte) blocks converted to int form)
	...
	for offset in range(0, len(msg), 64):
		...
		for i in range(64): # 1 pass through the loop processes some 32 bits out of the 512-bit block.
			if i < 16:
				...
				def func(b, c, d):
					return c ^ (b | ~d)
				index_func = lambda i: (7*i)%16  # noqa: E731
			...
			# we use 0xFFFFFFFF to ensure that the result is a 32-bit integer. 
...
if __name__ == '__main__':
	message = input()
	print([int(abs(math.sin(i+1)) * 4294967296) & 0xFFFFFFFF for i in range(64)])
	print(md5(message))
